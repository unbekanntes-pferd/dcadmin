<script lang="ts">
	import { roleListForGroups as roleList } from '$lib/users/models';
	import {
		Accordion,
		AccordionItem,
		getToastStore,
	...
	const fetchGroups = async () => {
		...
		try {
			...
			groupList = await getGroups(params);
			paginationSettings.size = groupList.range.total;
		} catch (e) {
	...
	const handleDownload = async () => {
		downloading = true;
		const defaultName = `groups-${todayStr}.csv`;
		...
		if (filePath) {
			...
			try {
				await downloadGroups(params, filePath);
	...
	const handleDownloadUsers = async () => {
		downloading = true;
		const defaultName = `group-users-${todayStr}.csv`;
		...
		if (filePath) {
			...
			try {
				await downloadAllGroupUsers(filePath);
				let successToast = createToastSettings(
					`Download of ${filePath} complete.`,
					ToastType.Success
				);
